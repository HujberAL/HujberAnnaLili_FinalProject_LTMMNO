...
model_a <- stan_glm(prey_p_month ~ age_years + hrs_indoors, data = cats_ref_dataset) # Creating the basic model
model_a_sum <- summary(model_a)
```
...
```{r}
model_b <- stan_glm(prey_p_month ~ age_years + hunt + hrs_indoors + n_cats, data = cats_ref_dataset) # Creating the model
model_b_sum <- summary(model_b)
```
